...
void setupGame (int *ptr , int *ptrStart , int *ptrEnd) ; // this sets up the full game

void randomNumber (int *targetNumber , int rangeStart , int rangeEnd) ; // it decides the target number

int hint (int *guessNumber , int *targetNumber , int *c , int rangeStart , int rangeEnd) ; // this give hint & warning msg
...
int main () {
    ...
    setupGame (&mode , &rangeStart , &rangeEnd) ;
    ...
    if (mode == 0) { // this is Classic Mode (Single Player) code ...
        ...
        do { // This takes input , checks if its correct guess & tracks the no of attemps too
            ...
            x = hint (&guessNumber , &targetNumber , &c , 1 , 100) ;
    ...
    if (mode == 1) { // this is Custom Mode (Single Player) code ...

        char playerName1[50] ;

        printf("\n========================================\n");
        printf("        SINGLE PLAYER - CUSTOM MODE\n");
        printf("========================================\n\n");
        ...
        printf("Fewer attempts means a higher score!\n");
        printf("Your best score will not be saved.\n\n");
        ...
        randomNumber (&targetNumber , rangeStart , rangeEnd) ; // this would pick a random number between the range
        ...
        do { // This takes input , checks if its correct guess & tracks the no of attemps too
            ...
            x = hint (&guessNumber , &targetNumber , &c , rangeStart , rangeEnd) ;
...
void setupGame (int *ptr , int *ptrStart , int *ptrEnd) {
...
int hint (int *guessNumber , int *targetNumber , int *c , int rangeStart , int rangeEnd) {

    if (*guessNumber < rangeStart || *guessNumber > rangeEnd) {
